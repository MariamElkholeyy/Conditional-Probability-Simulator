...
class ConditionalProbabilitySimulator:
    ...
    def calculate_conditional_probability(self, event_a, event_b, intersection_probability=None):
        probability_a = self.events[event_a]
        probability_b = self.events[event_b]
        
        if intersection_probability is None:
            # Assuming independent events, calculate intersection as product of probabilities
            intersection_probability = probability_a * probability_b

        return intersection_probability / probability_b
    ...
    while True:
        ...
        if choice == "1":
            ...
            if event_a in simulator.events and event_b in simulator.events:
                print(f"Checking events: {event_a}, {event_b}")
                independence_input = input(f"Are {event_a} and {event_b} independent? (yes/no): ")
                if independence_input.lower() == "yes":
                    # Independent events
                    probability = simulator.calculate_conditional_probability(event_a, event_b)
                    print(f"P({event_a}|{event_b}) = {probability:.4f}")
                else:
                    # Dependent events
                    intersection_input = input(f"Do you want to enter the intersection probability for P({event_a} ∩ {event_b})? (yes/no): ")
                    if intersection_input.lower() == "yes":
                        intersection_probability = float(input(f"Enter P({event_a} ∩ {event_b}): "))
                        probability = simulator.calculate_conditional_probability(event_a, event_b, intersection_probability)
                        print(f"P({event_a}|{event_b}) = {probability:.4f}")
                    else:
                        union_input = input(f"Do you want to enter the union probability for P({event_a} ∪ {event_b})? (yes/no): ")
                        if union_input.lower() == "yes":
                            union_probability = float(input(f"Enter P({event_a} ∪ {event_b}): "))
                            # Calculate intersection from union
                            intersection_probability = (simulator.events[event_a] + simulator.events[event_b]) - union_probability
                            probability = simulator.calculate_conditional_probability(event_a, event_b, intersection_probability)
                            print(f"P({event_a}|{event_b}) = {probability:.4f}")
                        else:
            ...
            if event_a in simulator.events and event_b in simulator.events:
                ...
                if intersection_input.lower() == "yes":
                    intersection_probability = float(input(f"Enter P({event_a} ∩ {event_b}): "))
                    conditional_probability = simulator.calculate_conditional_probability(event_a, event_b, intersection_probability)
                else:
